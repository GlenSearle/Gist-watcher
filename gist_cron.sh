...
curl -s -X GET \$(date +https://api.github.com/users/${git_user}/gists?since=%Y-%m-%dT%H:00:00Z -d  "${hours_ago} hours ago") | \
wc -l | \
grep 3 > /dev/null && \
date +%Y-%m-%dT%H:00:00Z > /var/log/last.gist
EOF
sudo chmod +x /etc/cron.${frequency}/gist-check-for-${git_user}
}

## This can be used if something more complicated needs to be triggered that isn't suitable for a one line bash scripts.
#if [ $? == 0]; then
#fi
